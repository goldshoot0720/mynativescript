...
export class Sub4Component implements OnInit {
  ...
  fetchData() {
    const url = "https://tsaopaofenghsiung2025-myangularsub4.firebaseio.com/.json";
  ...
  mytoFixed2(num: string): string {
    let num1 = Number(num.split("/")[0]);
    let num2 = Number(num.split("/")[1]);
    let num3 = (num1 / num2 * 100).toFixed(2);
    return String(num3)+"%";
  }

  mytoMinus(num: string): string {
    let num1 = Number(num.split("/")[0]);
    let num2 = Number(num.split("/")[1]);
    let num3 = num1-num2;
    return String(num3);
